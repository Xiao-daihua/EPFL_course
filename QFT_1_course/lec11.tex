...
\begin{align}
  \{\gamma^\mu,\gamma^\nu\}=2\eta^{\mu\nu}
\end{align}
我们可以根据Gamma矩阵的构造和之前的讨论验证这个运动方程是Lorentz Invariant的，我们根据Gamma矩阵的构造和之前讨论的(1/2,1/2)表示的结论有：
\begin{align}
...
对于这个变换我们有：
\begin{align}
  (i\ds{\partial}-m)\Psi_D(x) \to (i\gamma^\mu \partial_\mu' - m) \gamma^0 \Psi_D(Px) = \gamma^0 (i\gamma^\nu \partial_\nu - m) \Psi_D(Px)
\end{align}
...
\defi{
  ...
  \begin{align}
    S_D=\int d^4x\mathcal{L}_D=\int d^4x\mathrm{~}\bar{\Psi}_D(i\slashed{\partial}-m)\Psi_D, \quad \bar{\Psi}_D\equiv\Psi_D^\dagger\gamma^0
  \end{align}
}
给出的运动方程为Dirac方程：
\begin{align}
...
\end{align}
使用变分原理我们认为的独立变量其实是$ \psi_L $以及$ \psi^* $我们可以对于这两个量分别进行变分。对于质量项两个独立变量是decoupled的，第一项是$ \psi_L $的质量项，第二项是$ \psi_L^* $的质量项。但问题是如果我们看质量项的话会发现：
\begin{align}
  \psi_L^T\epsilon\psi_L = - \phi_L^T \epsilon^T \psi_L =  - \psi_L^T\epsilon\psi_L 
\end{align}
所以我们使用一般的数字是无法经典的写出一个Majorana Spinor的Lagrangian的！只能使用Grassmann数！我们变分也需要使用Grassmann number的规则。
...
\begin{itemize}
  \item SU(N)： $  \delta_{ij} $，$ \epsilon_{i_1 i_2 \cdots i_N} $ ；SU(2)Exp map形式：$  U(\theta) = e^{i\theta^i \sigma^i/2 } $
  \item SO(N) ：$ \delta_{ij} $，$ \epsilon_{i_1 i_2 \cdots i_N} $； SU(2) Triplet或者说SO(3)(注意，我们一般说SU(2) Triplet的时候其实是SO(3)的定义表示) Exp map 形式：$  R(\theta) = e^{i\theta^i J^i } $其中$ (J^i)^a_b = -i\epsilon^{iab} $
  \item U(1)：$ 1 $ Exp map形式：$  e^{i\alpha} $ 
  \item U(2)：$ \delta_{ij} $ ；Exp map形式：$  U(\theta,\phi) = e^{i\theta^\mu \sigma^\mu/2 } e^{i\phi} $这里有一个I的加入！
  \item O(N)：$ \delta_{ij} $ 
\end{itemize}
...
如果我们存在两套场按照某一个Internal Symmetry的两个表示变换。我们可以类似Dirac Spinor构造Lorantz变换Defining Representation变换的量一样，通过某个internal Symmetry的某个表示的多个场组合出按照另一个表示变换的量（其实就是一种张量积表示约化）。
