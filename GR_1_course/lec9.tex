...
\end{align}
注意，这里$ \hat{\mathbf{n}} = \frac{\mathbf{x}}{|\mathbf{x}|} $是观察者位置的单位向量。因为作为观察者，我们只能观测到引力波向自己方向传播的部分！
...
我们补充上一小节的内容。如果我们考虑一个任意的质量$ M(x) $分布变换生成的引力波。然后，我们假设观察者的位置在这个坐标系下为 $ \mathbf{x} = R \hat{z} $。于是，我们关心z方向传播的引力波的quadral pole表示。也就是我们考虑投影算符之中$ \hat{n} = (0,0,1) $的情况。我们有：
\begin{align}
  P_{ij} = \delta_{ij} - \hat{n}_i \hat{n}_j = \begin{pmatrix}1&0&0\\0&1&0\\0&0&0\end{pmatrix},
\end{align}
根据投影算符公式$ \Lambda_{ij,kl} = P_{ik}P_{jl} - \frac{1}{2}P_{ij}P_{kl} $，我们使用下面矩阵形式进行计算。对于投影算符作用在一般的矩阵$ A_{ij} $上面我们有：
\begin{align}
  \Lambda_{ij,kl}A_{kl}=\left[P_{ik}P_{jl}-\frac{1}{2}P_{ij}P_{kl}\right]A_{kl}=(PAP)_{ij}-\frac{1}{2}P_{ij}\mathrm{Tr}\left(PA\right).
\end{align}
带入最一般的$ \ddot{M}_{ij}(x) $的形式我们知道：
\begin{align}
  \Lambda_{ij,kl}\ddot{M}_{kl}=\begin{pmatrix}(\ddot{M}_{11}-\ddot{M}_{22})/2&\ddot{M}_{12}&0\\\ddot{M}_{21}&-(\ddot{M}_{11}-\ddot{M}_{22})/2&0\\0&0&0\end{pmatrix}_{ij}
\end{align}
...
\begin{align}
  h_{ij}(t,R\hat{z})=\frac{2G}{R}\frac{d^2}{dt^2}\begin{pmatrix}M_{11}(t-R)-M_{22}(t-R)&2M_{12}(t-R)&0\\2M_{21}(t-R)&-(M_{11}(t-R)-M_{22}(t-R))&0\\0&0&0\end{pmatrix}_{ij}.
\end{align}
这个和我们之前讨论的$ h_+, h_\times $的形式完全相符，说明我们讨论是合理的。我们分别给出这两个Polarization对应的quadrupole矩阵形式：
\begin{align}\label{eq:gwquadrupolepluscross}
  h_{+}&=\frac{G}{R}\left(\ddot{M}_{11}-\ddot{M}_{22}\right), \quad h_{\times}=\frac{2G}{R}\ddot{M}_{12}\mathrm{~,}
\end{align}
...
\thm{\label{thm:gwpowerquadruple}
  ...
  引力波的辐射功率和源的关系为：
  \begin{align}
    P_{quad} = \int d \Omega \frac{G}{8 \pi} \Lambda_{ij,kl}(\hat{\mathbf{n}}) \langle \dddot{Q}^{ij}(t - R) \dddot{Q}^{kl}(t - R) \rangle = \frac{G}{5} \langle \dddot{Q}_{ij}(t - R) \dddot{Q}^{ij}(t - R) \rangle.
  \end{align}
}
注意，这个公式是对于所有方向传播的引力波的传播方向的面积进行积分得到的总功率！！这个积分结果可以通过Quadralpole moment的三阶导数的contract得到。我们注意，这个cointract之后，如果固定观察者的位置$ R $这是一个仅仅和时间相关的量。我们对于时间平均之后得到的了一个常数功率！
