...
测量一个量子系统对于某个可观测量$ M $的期望值的时候我们从不可以只测一次，需要进行多次测量然后取平均得到。我们称多次测量平均的结果是Estimator，也就是一个期望值的估量，同时也是一个随机变量。我们下面希望研究这个随机变量在什么程度上可以近似真实的期望值，同时也希望定量的描述其误差。
...
\defi{
  ...
  对于一个系统$ \rho $，我们希望测量某个观测量$ M $的期望值$ \langle M \rangle = \mathrm{Tr}(\rho M) $。我们在$ M = \sum_n^d \lambda_n \ketbra{n}{n} $这组基构成的POVM下进行测量。我们制备$ N $份同样的系统$ \rho $每一份进行一次测量，对于第$ s $次测量给出的结果为$ \lambda_{ms} $。我们定义Estimator为：
  \begin{align}
    M_N = \frac{1}{N}\sum_{s=1}^N \lambda_{m(s)}\mathrm{~.}
  \end{align}
}
对于这个随机变量我们可以定义一个期望值和方差来描述其性质。期望值为对于所有可能的$ \{\lambda_{m{s}}\} $构型，以一定概率进行加权的平均值。方差则是每一种构型的平方的期望值减去期望值的平方。
...
\rmk{
  注意区分第二个和第三个等式：第二个等式说的是独立随机变量可以直接求和；第三个等式说的是如果是两个随机变量正好两个一模一样，那么方差就会变成$ 4\mathrm{Var}(X) $。所以并不矛盾。
}
...
\rmk{
  对于测量一个系统，我们设计一个Estimator，我们需要保证其是unbiased的（也就是期望值等于$ \tr (\rho M) $）。同时我们希望其方差尽可能小，这样才能保证测量结果的可靠性。因此我们需要验证期望值以及尽量减小方差。
}
