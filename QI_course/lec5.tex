...
观察这个结果，我们会发现这个更大系统的Unitary配上合适的环境的初始状态，我们可以得到一个partial dephasing的效果下面我们证明一下，上面的结果$ \mathrm{Tr}_E\left(U(\rho\otimes|0\rangle\langle0|)U^\dagger\right) = \sigma=p\rho+(1-p)Z\rho Z $成立，我们直接进行计算：
\begin{align}
...
\begin{align}
  U(|\psi\rangle\otimes|0\rangle_E)=\sum_i   A_i |\psi\rangle \otimes |i\rangle_E
\end{align}
...
\end{align}
所以我们最终呈现的Unitary矩阵一定是，第一列是$ A_i $后面的列待定。我们可以根据Unitary的条件确定后面列的数值。数学上写出来就是：
\begin{align}
  U = \sum_i A_i \otimes |i\rangle\langle0| + V,
\end{align}
其中$ V $是补全Unitary矩阵的部分。保证这个矩阵是Unitary的！
...
\subsection{Choi Representation I}
...
证明方法就是写作$ \ket{Vec(ABC)} $的形式，但是由于有的矩阵就是Identity所以可以随便换顺序，因此给出了上面的结果。
  \item Usage of Ricochet Identity:
    ...
    比如我们可以推导出来这样的一个结果：
    \begin{align}
      \left(UV^{\dagger}\otimes I\right)|\phi^{+}\rangle=(U\otimes V^{*})|\phi^{+}\rangle.
    \end{align}
...
为了方便计算这个定义写开来就是：
\begin{align}
  J(\mathcal{E})=\sum_{i,j}(\mathcal{E}\otimes I)|ii\rangle\langle jj|=\sum_{i,j}\mathcal{E}(|i\rangle\langle j|)\otimes|i\rangle\langle j|.
\end{align}
也就是第二个system的$ \ketbra{i}{j} $部分纠缠上了第一个system的$ \mathcal{E}(|i\rangle\langle j|) $部分。

\bigskip
\hlr{Kraus Operator表示的Channel的Choi State}

显然对于Kraus Channel我们可以计算出来Choi State的形式，来考虑下面这样的一个Kraus Operators：$ \{ A_i \} $通过计算我们知道的结论是：
\thm{
  Choi State和Kraus Operators的关系

  对于某个Quantum Channel一组Kraus Operators $ \{A_i\} $，Quantum Channel对应的Choi State可以写作：
\begin{align}
  J(\mathcal{E}) = \sum_i |Vec(A_i)\rangle\langle Vec(A_i)|.
\end{align}
}
Proof：我们可以直接使用Vectorization的性质以及Kraus Operator的定义，会发现：
\begin{align}
  A_i \otimes I |Vec(I)\rangle = |Vec(A_i)\rangle.
\end{align}
带入Choi State的定义我们就可以得到上面的结果。
\rmk{
...
其实这个命题的反命题也是成立的，也就是说给定Choi State我们可以通过对角化得到Kraus Operators。
\thm{\label{thm:choi-to-kraus}
  Choi State的对角化得到Kraus Operators

  对于一个Quantum Channel对应的Choi State $ J(\mathcal{E}) $，我们可以通过对其进行对角化得到一组本征值和本征向量：
  \begin{align}
    J(\mathcal{E})=\sum_i\lambda_i|v_i\rangle\langle v_i|. 
  \end{align}
  然后我们把本征向量重新写成$ |Vec(A_i)\rangle $的形式，然后使用vectorization的反操作就可以得到一组算符：
  \begin{align}
    \ket{Vec(A_i)} = \sqrt{\lambda_i} |v_i\rangle \quad \rightarrow \quad A_i.
  \end{align}
  这一组算符必然是这个Quantum Channel一组合法的Kraus Operators。
}
证明：应该可以使用Choi State的定义进行计算给出。
\YL{[我懒了呜呜呜呜但是还是挺重要的捏]}


\rmk{
  这个定理里面需要对于Choi State进行对角化，这是可以操作的，因为一个合理的Quantum Channel需要是Complete Positive的，所以Choi State必然是一个半正定矩阵(自然的也是Hermite的)，因此我们可以对角化一组正实数的本征值和一组正交归一化的本征向量。
}
\rmk{
  注意，Choi State虽然一定是半正定矩阵，但是其不一定是一个合法的密度矩阵，因为$ \tr J(\mathcal{E}) \neq 1 $。一般我们并不进行归一化。
}

\bigskip
\hlr{Choi State给出Quantum Channel}

Generally，其实我们可以通过Choi State来唯一地确定一个Quantum Channel。我们给出下面的定理：
\thm{
Choi State唯一确定Quantum Channel

  对于给定的Choi State $ J(\mathcal{E}) $，我们可以通过下面的公式唯一地确定一个Quantum Channel：
  \begin{align}
    \mathcal{E}(\rho)=\mathrm{Tr}_B\left(\left(I\otimes\rho^T\right)J(\mathcal{E})\right).
  \end{align}
}
