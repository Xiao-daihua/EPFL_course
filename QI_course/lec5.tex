...
\thm{
  ...
  对于一组Kraus Operators $ \{A_i\} $给出的quantum channel，对应的Choi State可以写作：
\begin{align}
...
\thm{
  ...
  对于一个Choi State $ J(\mathcal{E}) $，我们可以通过对其进行对角化得到一组本征值和本征向量：
  \begin{align}
    J(\mathcal{E})=\sum_i\lambda_i|v_i\rangle\langle v_i|. 
  \end{align}
  然后我们把本征向量重新写成$ |Vec(A_i)\rangle $的形式，然后使用vectorization的反操作就可以得到Kraus Operator：
  \begin{align}
    \ket{Vec(A_i)} = \sqrt{\lambda_i} |v_i\rangle \quad \rightarrow \quad A_i.
  \end{align}
}
这个证明需要使用下面Choi State和Quantum Channel的关系。
\rmk{
