...
在tr之中我们可以进行随意的\textbf{循环换顺}，也就是说：
\begin{align}
...
对于一个量子态$ \rho_{AB} = \rho_A \otimes \rho_B $，我们有：
\begin{align}
  \mathrm{Tr}_A(\rho_{AB}) = \mathrm{Tr}_A(\rho_A \otimes \rho_B) =
  (\mathrm{Tr}_A \rho_A) \rho_B = \rho_B
\end{align}
...
\begin{align}
  \mathrm{Tr}(\rho)=\sum_{a,b}f(a,b)\operatorname{Tr}(|a\rangle\langle
  b|)=\sum_{a,b}f(a,b)\left\langle b|a\right\rangle
\end{align}

\bigskip
\hlr{Trace和Unitary演化}

如果我们有A,B两个系统，然后在其上有一个$ \rho_{AB}
$。如果在B系统上面进行Unitary演化以及A系统上面进行任意演化，我们发现这两个过程是可以互换的。所以特殊的对于Partial Trace：
\begin{align}
  Tr_A ((I_A \otimes U_B) \rho_{AB} (I_A \otimes U_B^\dagger)) = U_B
  Tr_A(\rho_{AB}) U_B^\dagger
\end{align}
...
\begin{align}
  ...
  \\
  &\mathrm{Tr}(\rho^2)\leq1,\quad\mathrm{Tr}(\sigma^2)\leq1
  \Rightarrow \mathrm{Tr}(\rho\sigma)\leq1
\end{align}
...
\bigskip
\hlr{Super Operator 和 Trace}

对于一个Super Operator $ \mathcal{E} $，我们会发现：
\begin{align}
  \mathrm{Tr}[\mathcal{E}(\rho)O]=\mathrm{Tr}[\rho\mathcal{E}^\dagger(O)]
\end{align}
对于superoperator的dagger，我们称之为adjoint。首先对于superoperator可以写作：$
\mathcal{E}(X)=\sum_kA_kXB_k^\dagger $，那么他的adjoint定义为：
\begin{align}
  \mathcal{E}^\dagger(X)=\sum_kA_k^\dagger XB_k
\end{align}
...
\hlr{Delta函数的求和}
\begin{align}
  \sum_{ij} \delta_{ij} \delta_{ij} = \sum_i 1 = d
\end{align}
但是如果对于插入了系数：
\begin{align}
  \sum_{ij} \delta_{ij} A_{ij} = \sum_i A_{ii} = \mathrm{Tr}(A) \quad
  \sum_{ij} \delta_{ij} \delta{ij} a_i = \sum_i a_i
\end{align}
...
更严格地，利用谱分解 $P=\sum_i \lambda_i\ket{i}\bra{i}$，并注意 $\rho$ 为正定且
$\operatorname{Tr}\rho=1$，可得
\[
  \langle P\rangle_\rho = \sum_i \lambda_i p_i,\qquad p_i\ge 0,\quad
  \sum_i p_i=1,
\]
即为 $\pm 1$ 的加权平均，从而有
\[
...
\begin{itemize}
  ...
  \item Bell State 其实都是正比于Pauli Matrix 的 Vectorization 形式：
    \begin{align}
      |\Phi^+\rangle = \frac{1}{\sqrt{2}} |Vec(I)\rangle,\quad&
      |\Phi^-\rangle = \frac{1}{\sqrt{2}} |Vec(Z)\rangle, \\
      |\Psi^+\rangle = \frac{1}{\sqrt{2}} |Vec(X)\rangle,\quad&
      |\Psi^-\rangle = \frac{i}{\sqrt{2}} |Vec(Y)\rangle.
    \end{align}
\end{itemize}
