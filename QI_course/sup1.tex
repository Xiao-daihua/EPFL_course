...
\begin{align}
  \mathrm{Tr}_A(\rho_{AB}) = \mathrm{Tr}_A(\rho_A \otimes \rho_B) = (\mathrm{Tr}_A \rho_A) \rho_B = \rho_B
\end{align}
...
\end{align}
对于superoperator的dagger，我们称之为adjoint。首先对于superoperator可以写作：$ \mathcal{E}(X)=\sum_kA_kXB_k^\dagger $，那么他的adjoint定义为：
\begin{align}
...
但是如果对于插入了系数：
\begin{align}
  \sum_{ij} \delta_{ij} A_{ij} = \sum_i A_{ii} = \mathrm{Tr}(A) \quad \sum_{ij} \delta_{ij} \delta{ij} a_i = \sum_i a_i
\end{align}
